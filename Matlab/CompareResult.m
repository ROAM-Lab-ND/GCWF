...
sampleSet = "TestingResult_6Vertices/";
...
disp("Grid Search 5000");
GD5K = loadTestData_Propose(sampleSet+'Tau_Grid_5000.txt', CWC);

load(sampleSet+"num_GD.mat");
disp("Grid Search "+num2str(num_GD));
GD = loadTestData_Propose(sampleSet+'Tau_Grid_'+num2str(num_GD)+'.txt', CWC);


Method = ["2nd CWC","1st CWC-4","1st CWC-8","Approach 1","Approach 2","GD-"+num2str(num_GD),"Optimal Line"]';
...
STD = [0,std(CWCLin_4.err_whole_NoNan),std(CWCLin_8.err_whole_NoNan),std(LCL.err_whole),...
    std(EAV.err_whole),std(GD.err_whole),std(GD5K.err_whole)]';
table(Method,ErrorRate,ComputationTime,STD)
...
legend_strs = plotHist(GD.err_whole,'x-',legend_strs,"Grid Search with "+num2str(num_GD)+" Grids");
...
exportgraphics(f,'ErrorCDF.pdf','BackgroundColor','none','ContentType','vector')
